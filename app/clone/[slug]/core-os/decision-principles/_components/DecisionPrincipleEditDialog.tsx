...
interface Props {
  ...
  tenantId: string;
  principleId: string;
  initial: DecisionPrincipleInput;
}
...
export function DecisionPrincipleEditDialog({
  slug,
  tenantId,
  principleId,
  initial,
  ...
  const [form, setForm] = useState<DecisionPrincipleInput>(initial);
  const [error, setError] = useState<string | null>(null);
  const hasOptional = Boolean(initial.exception || initial.related_values);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await updateDecisionPrinciple(
        slug,
        tenantId,
        principleId,
        form,
