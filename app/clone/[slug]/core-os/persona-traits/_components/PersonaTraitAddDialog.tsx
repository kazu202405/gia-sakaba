...
// AI 抽出を待たず手動で「自分のクセ」を書く用のダイアログ。
// 初期 status は 'adopted'（自分で書いたものは即採択扱い）。

import { useState, useTransition } from "react";
import { Plus, X, Loader2, AlertCircle } from "lucide-react";
import { PERSONA_TRAIT_CATEGORIES } from "@/lib/ai-clone/supabase-db";
import { createPersonaTrait } from "../_actions";
...
export function PersonaTraitAddDialog({ slug, tenantId }: Props) {
  const [open, setOpen] = useState(false);
  const [category, setCategory] = useState<string>(PERSONA_TRAIT_CATEGORIES[0]);
  ...
  const reset = () => {
    setCategory(PERSONA_TRAIT_CATEGORIES[0]);
    setTrait("");
    setDetail("");
    setError(null);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await createPersonaTrait(
        slug, tenantId,
        { category, trait, detail: detail || null },
        "adopted", // 手動追加は即採択
      );
      if (!res.ok) {
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-[#1c3550] text-white text-xs font-bold tracking-[0.06em] hover:bg-[#0f2238] transition-colors"
      >
          ...
          <div className="relative w-full max-w-lg bg-white border border-gray-200 rounded-md shadow-xl max-h-[calc(100vh-2rem)] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="px-5 py-5 space-y-5">
              ...
              <div className="flex items-center justify-end gap-2 pt-2 border-t border-gray-100">
                ...
                <button
                  type="submit"
                  disabled={pending || trait.trim().length === 0}
