...
import { createExpense, type ExpenseInput } from "../_actions";
...
const FIXED_OR_VARIABLE_OPTIONS = [
  { value: "", label: "未設定" },
  { value: "固定", label: "固定" },
  { value: "変動", label: "変動" },
];
...
const buildEmptyForm = (): ExpenseInput => ({
...
export function ExpenseAddDialog({ slug, tenantId }: Props) {
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<ExpenseInput>(buildEmptyForm);
  ...
  const change = <K extends keyof ExpenseInput>(
    key: K,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await createExpense(slug, tenantId, form);
      if (!res.ok) {
          ...
          <div className="relative w-full max-w-lg bg-white border border-gray-200 rounded-md shadow-xl max-h-[calc(100vh-2rem)] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="px-5 py-5 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className={labelClass}>固定/変動</label>
                  <select
                    value={form.fixed_or_variable ?? ""}
                    onChange={(e) =>
                      change("fixed_or_variable", e.target.value)
                    }
                    className={inputClass + " bg-white"}
                  >
                    {FIXED_OR_VARIABLE_OPTIONS.map((o) => (
