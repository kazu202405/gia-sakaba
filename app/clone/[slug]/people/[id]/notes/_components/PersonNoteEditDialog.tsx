...
import { updatePersonNote, type PersonNoteInput } from "../_actions";
...
export function PersonNoteEditDialog({
  slug,
  tenantId,
  personId,
  noteId,
  ...
  const [form, setForm] = useState<PersonNoteInput>(initial);
  const [error, setError] = useState<string | null>(null);
  const hasOptional = Boolean(
    initial.challenges || initial.caveats || initial.interests,
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await updatePersonNote(
        slug,
        tenantId,
        personId,
        noteId,
