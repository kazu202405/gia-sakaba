...
// 案件進捗ログ追加ダイアログ。occurred_at 必須、初期値 = 現在時刻。
// 進捗内容・現在の状態・課題・次のアクション・判断が必要なこと。

import { useState, useTransition } from "react";
import { Plus, X, Loader2, AlertCircle, ChevronDown } from "lucide-react";
import { createProgressLog, type ProgressLogInput } from "../_actions";
...
const buildEmptyForm = (): ProgressLogInput => ({
...
export function ProgressLogAddDialog({ slug, tenantId, projectId }: Props) {
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<ProgressLogInput>(buildEmptyForm);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await createProgressLog(slug, tenantId, projectId, form);
      if (!res.ok) {
    ...
    <>
      ...
      {open && (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="progress-add-title"
