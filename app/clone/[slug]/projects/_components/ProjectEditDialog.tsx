...
// /clone/[slug]/projects/[id] の編集ダイアログ。ProjectAddDialog と同構造、initial で初期化、updateProject を呼ぶ。

import { useState, useTransition } from "react";
import { Pencil, X, Loader2, AlertCircle, ChevronDown } from "lucide-react";
import { updateProject, type ProjectInput } from "../_actions";
...
export function ProjectEditDialog({
  ...
  const hasOptionalValues = Boolean(
    initial.revenue_total ||
      initial.cost_total ||
      initial.hours_invested ||
      initial.pending_decision,
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await updateProject(slug, tenantId, projectId, form);
      if (!res.ok) {
