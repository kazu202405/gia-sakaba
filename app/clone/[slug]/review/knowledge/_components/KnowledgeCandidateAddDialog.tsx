...
const emptyForm: KnowledgeCandidateInput = {
  content: "",
  kind: "",
  target_db: "",
  priority: "",
...
export function KnowledgeCandidateAddDialog({ slug, tenantId }: Props) {
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState<KnowledgeCandidateInput>(emptyForm);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await createKnowledgeCandidate(slug, tenantId, form);
      if (!res.ok) {
            ...
            <form onSubmit={handleSubmit} className="px-5 py-5 space-y-5">
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  placeholder="これは type 化できそうな知見・FAQ・トーク（タイトルでも本文でもOK）"
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  placeholder="どの会話・案件・判断から拾った候補か（後で辿れるように）"
                  className={inputClass + " resize-y"}
                />
              </div>
