...
export function ServiceDeleteButton({
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const res = await deleteService(slug, tenantId, serviceId);
      if (!res.ok) {
        setError(res.error ?? "削除に失敗しました");
        return;
      }
      router.push(`/clone/${slug}/services`);
      router.refresh();
    });
  };
          ...
          <div className="relative w-full max-w-md bg-white border border-gray-200 rounded-md shadow-xl">
            ...
            <div className="px-5 py-5 space-y-4">
              <p className="text-sm text-gray-700 leading-relaxed">
                <span className="font-bold text-[#1c3550]">{serviceName}</span>{" "}
                を削除します。
              </p>
              <p className="text-[12px] text-gray-500 leading-relaxed">
                関連リンクもすべて削除されます。元に戻すことはできません。
              </p>
