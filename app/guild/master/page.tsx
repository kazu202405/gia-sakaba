...
import { getGuildContext, listGuildIntroRequests, listGuildInviteNetwork, listGuildMasterInvites, listGuildMembers, listPendingGatheringApplications } from "@/lib/guild/server-data";
...
export default async function MasterPage() {
  ...
  const [requests, members, pendingGatherings, invites, network] = await Promise.all([
    listGuildIntroRequests(), listGuildMembers(), listPendingGatheringApplications(), listGuildMasterInvites(), listGuildInviteNetwork(),
  ]);
  return (
    <div className="space-y-9">
      <PageTitle
        title="ギルドマスター"
        lead="紹介依頼・限定の集まり・招待のつながりをまとめて管理します。"
      />
