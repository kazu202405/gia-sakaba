...
export function InviteSignup({ inviteCode, inviterName, preview = false }: { inviteCode: string; inviterName: string; preview?: boolean }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[1fr_0.72fr] lg:items-start">
    <Window title="はじめての方">
      ...
      <p className="mb-5 text-sm leading-relaxed"><span className="tracking-wider">{inviterName || "酒場のメンバー"}さんからの招待です。</span><br /><span className="c-muted">GIA共通アカウントを作って、入会手続きへ進みます。</span></p>
      <form onSubmit={submit} className="space-y-4">
        ...
        <button type="submit" disabled={busy || preview} className="rpg-button h-12 w-full text-sm disabled:opacity-50">{preview ? "プレビュー中（送信できません）" : busy ? "作成中…" : "▶ アカウントを作って続ける"}</button>
      </form>
    </Window>
    <Window title="アカウントをお持ちの方">
      <p className="c-muted mb-5 text-sm leading-relaxed">すでにGIAで登録したメールアドレスがある方は、ログインするとこの招待へ戻れます。</p>
      {preview ? <span className="c-button-sub inline-flex h-12 w-full items-center justify-center text-sm opacity-50">ログインして続ける</span> :
        <Link href={`/guild/login?next=${encodeURIComponent(joinPath)}`} className="c-button-sub inline-flex h-12 w-full items-center justify-center text-sm">ログインして続ける</Link>}
    </Window>
  </div>;
}
