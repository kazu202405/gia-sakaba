...
export function LiveProjectDetail({ project, canEdit }: { project: GuildProject; canEdit: boolean }) {
  ...
  async function addTask(event: React.FormEvent) {
    ...
    else { setTaskTitle(""); router.refresh(); }
    setBusy(false);
  }
    ...
    <section className="c-window p-5 pt-10 sm:p-7 sm:pt-11">
      ...
      {canEdit && <div className="mt-5 flex flex-wrap gap-3">
        ...
        <button type="button" disabled={busy} onClick={() => setProjectStatus(project.status === "done" ? "active" : "done")} className="c-button-sub px-4 py-2">
          {project.status === "done" ? "進行中に戻す" : "プロジェクトを完了する"}
        </button>
      </div>}
    </section>
    <section className="c-window p-5 pt-10 sm:p-7 sm:pt-11">
      ...
      {canEdit && project.status === "active" && <form onSubmit={addTask} className="mt-5 flex gap-2">
        ...
        <button type="submit" disabled={busy || !taskTitle.trim()} className="rpg-button shrink-0 px-4 disabled:opacity-50">追加</button>
      </form>}
