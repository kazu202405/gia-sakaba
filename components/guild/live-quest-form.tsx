...
export function LiveQuestForm({ questTerm, gathering = false }: { questTerm: string; gathering?: boolean }) {
  const router = useRouter();
  const [draft, setDraft] = useState<Draft>(gathering ? { ...EMPTY, category: "gathering" } : EMPTY);
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="mb-9"><BackLink href={gathering ? "/guild/master" : "/guild/quests"} label={gathering ? "ギルドマスター" : `${questTerm} けいじばん`} /></div>
      <section className="c-window p-5 pt-10 sm:p-7 sm:pt-11">
        <span className="c-window-title">{step === "form" ? gathering ? "限定の集まりを開く" : `${questTerm}を出す` : "かくにん"}</span>
        {step === "form" ? (
          <>
            <h1 className="text-xl tracking-wider">{gathering ? "どんな集まりを開きますか？" : `どんな${questTerm}を出しますか？`}</h1>
            <p className="c-muted mt-2 text-sm leading-relaxed">{gathering ? "有料会員が詳しい内容を見て申し込めます。はじめて申し込む人はギルドマスターの承認が必要です。" : "投稿すると、酒場のメンバーに公開されます。"}</p>
            <div className="mt-7 space-y-6">
              {gathering ? <p className="c-card px-3 py-2.5 text-sm">{questCategoryMark.gathering} {questCategoryLabel.gathering}<span className="c-chip ml-2">有料会員限定</span></p> : <Field label="しゅるい" required error={errors.category}>
              ...
              <Field label="タイトル" required error={errors.title}>
                <TextInput value={draft.title} onChange={(value) => set("title", value)} max={40} label="タイトル" placeholder={gathering ? "例：経営者どうしの少人数交流会" : undefined} />
              </Field>
              ...
              <Field label="くわしく" required hint={gathering ? "開催日時・会場・当日の流れを書いてください" : undefined} error={errors.body}>
                <TextArea value={draft.body} onChange={(value) => set("body", value)} rows={6} max={800} label="くわしく" />
              </Field>
              <p className="c-card border-dashed px-3 py-2.5 text-xs leading-relaxed">{gathering ? "※ 酒場内で会費は集められません。当日お店で払う会費があれば、詳しい内容に書いてください。" : "※ 金額（報酬・予算）はここには書かず、会って話すときに決めてください。"}</p>
          ...
          <>
            <p className="c-muted text-sm">{gathering ? "この内容で、有料会員向けの集まりを公開します。" : "この内容で公開します。"}</p>
            <div className="c-card mt-5 space-y-3 p-4 sm:p-5">
              <p className="c-label text-xs">{questCategoryMark[draft.category as QuestCategory]} {questCategoryLabel[draft.category as QuestCategory]}{gathering && <span className="c-chip ml-2">有料会員限定</span>}</p>
            ...
            <div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
              ...
              <button type="button" onClick={submit} disabled={saving} className="rpg-button h-12 px-6 text-base">{saving ? "公開中…" : "▶ 公開する"}</button>
            </div>
          </>
        )}
      </section>
