// 8bitcn/ui の 8-bit Button を写したもの（MIT・LICENSE.md 参照）。
// 変更点：このプロジェクトに無い `radix-ui` の代わりに、入っている `@radix-ui/react-slot` から Slottable を読む。
...
export interface BitButtonProps extends ButtonHTMLAttributes<HTMLButtonElement>, VariantProps<typeof buttonVariants> {
  asChild?: boolean;
...
function ButtonDecorations({ size, variant }: ButtonDecorationsProps) {
  return (
    <span aria-hidden="true" className="pointer-events-none contents" data-slot="button-decorations">
...
function Button({ asChild = false, children, className, font, size, variant, ...props }: BitButtonProps) {
